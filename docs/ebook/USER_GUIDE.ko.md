...
4. 새 책의 표지(크림색 바탕·짙은 녹색 글자), 미색 본문, 고딕 글꼴, 짙은 녹색 문제 번호, 황토색 정답 띠, 한 톤 짙은 미색 해설 영역을 확인합니다. 본문의 `문제로 돌아가기`, `정답 확인`, 반복 이동 링크가 없어야 합니다.
...
- 이번 본문 디자인은 **라이트 모드 전용**입니다. 미색 바탕과 짙은 글자색을 기본으로 지정합니다. 읽기 앱에서 어두운 테마를 강제하면 책의 색이 달라질 수 있으므로 라이트 테마에서 확인해 주세요. EPUB이 외부 앱의 테마 선택까지 제한하지는 못합니다.
...
- 따뜻한 미색 바탕에 고딕 본문을 사용합니다. 문제 번호는 크게, 출처는 작고 차분하게 표시합니다.
- 정답은 황토색 띠에, 해설 전체는 조금 더 짙은 미색 영역에 표시합니다. 보기별 해설은 같은 영역 안에서 제목과 구분선으로 나눕니다.
