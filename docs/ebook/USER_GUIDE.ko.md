...
4. 새 책의 표지(크림색 바탕·짙은 녹색 글자), 문제 구분선, 간결한 정답 영역을 확인합니다. 본문의 `문제로 돌아가기`, `정답 확인`, 반복 이동 링크가 없어야 합니다.
...
- 검은 배경이 싫으면 같은 설정에서 **라이트 모드**를 선택합니다. 배경은 책의 오류가 아니라 앱의 테마 설정입니다.
...
개발 검수 샘플은 `yusin-quiz/build/ebook-samples/sample-review.epub`에 생성됩니다. 실제 기출 데이터가 아닌 가상 문항이며, 본문 XHTML의 밝은/어두운 화면과 큰 글씨는 별도 렌더링으로 확인합니다. 실제 Mac 도서 앱과 Windows 리더에서 최종 검수해 주세요.
