...
Statement-group correction: `dl.statement` now groups each `dt`/`dd` pair in `.statement-item`. Float the label in a 2.2em column, indent content 2.4em, reset first/last block margins, and clear each item. This keeps labels such as ㄱ. on the first text line and aligns wrapped lines with the content, while preserving nested blocks and allowing long entries to paginate. All nine EPUB placement combinations still pass EPUBCheck. The supplied four-label example was checked in light/dark and 390px/24px browser renders; label/content top offsets are zero and no horizontal overflow occurs. This does not claim native Books pagination verification.

## Warm light reading revision — 2026-09-19

The current user interview supersedes the earlier serif, unset body colors, and dark-preview direction. Confirmed: EPUB across phone/tablet/PC and both orientations; balanced information density; Korean sans-serif defaults; light-only warm ivory canvas with a slightly deeper ivory explanation surface. Workbook references inform hierarchy rather than fixed print columns. See the replaced scoped `DESIGN.md` for exact built values.

- `reading.css`: paired ivory/ink colors; local sans-serif fallbacks; 90% body width capped at 38em; body line height 1.8; differentiated chapter/number/source/section type scale; balanced paragraph, choice and question spacing; warm ochre answer strips; continuous ivory explanation panels with per-choice subheadings. No authored dark theme, fixed-height containers, or forced columns.
- `EpubRenderer.java`: separates original question number, number unit, and standalone entry kind into semantic spans; generated inline explanation labels receive dedicated classes. Source wording, stored emphasis, placement settings, links/IDs, and the earlier statement hanging-indent fix are retained. No API/admin/mobile change.
- `EpubRendererTest`: human-review outputs now use clearly fictional long Korean editorial content (not laws or actual exams) and include both `sample-review.epub` (year-end explanations) and `sample-reading-inline.epub` (inline explanations). Existing behavioral/EPUBCheck fixtures remain intact.
- `USER_GUIDE.ko.md`: explains the new appearance, regeneration, sample files, and the boundary that an external reader can override book colors or fonts. Light-only author styling does not disable an external app's theme controls.
